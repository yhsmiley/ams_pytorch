...
# def _get_val_item(ds, idx):
#     img, target = ds[idx]
#     bboxes_with_cls_id = np.array(target)
...
#     # use area to filter out placeholder targets
#     area_idx = np.where(target['area'] == 0)
#     if len(area_idx[0]):
#         target['boxes'] = target['boxes'][:area_idx[0][0]]
#         target['labels'] = target['labels'][:area_idx[0][0]]
#         target['area'] = target['area'][:area_idx[0][0]]
#         target['iscrowd'] = target['iscrowd'][:area_idx[0][0]]
...
def convert_to_coco_api(ds, bbox_fmt='voc'):
    ...
    for img_idx in range(len(ds)):
        ...
        # img, targets = _get_val_item(ds, img_idx)
